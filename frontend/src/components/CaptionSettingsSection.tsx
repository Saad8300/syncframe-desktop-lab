    ...
    <div className={`space-y-3 ${disabled ? 'opacity-50 pointer-events-none' : ''}`}>
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-3">
          ...
          {/* Uses the shared .form-select primitive rather than bespoke styling.
              The previous hand-rolled version set a semi-transparent background
              (bg-black/20 in dark mode); Chromium renders the OS option popup
              from the select's own background colour, so 20%-alpha black
              composited to a pale panel with unreadable text in Dark Mode.
              .form-select supplies an opaque background, explicit
              option/optgroup colours, and the standard chevron. The sizing
              overrides are safe now that the primitive pins line-height
              rather than height. */}
          {!hideSource && (
            <select
              value={config.source}
              ...
              className="form-select w-auto text-[11px] font-semibold py-1 pl-3"
            >
